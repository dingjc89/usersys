...
<form id="form" action="{{url('admin/role/update').'/'.$role->role_id}}" method="post" class="wizard-big wizard clearfix" role="application" novalidate="novalidate">
    {{csrf_field()}}
    <input type="hidden" name="pid" value="{{$role->pid}}">
    <input type="hidden" name="path" value="{{$role->path}}">
        ...
        <div class="row">
            <div class="col-sm-8">
                <div class="form-group">
                    <label>角色 *</label>
                    <input id="roleName" name="roleName" value="{{$role->name}}" type="text" class="form-control required" aria-required="true" />
                </div>
            </div>
            <div class="col-sm-4">
                    ...
                    <select class="chosen-select form-control" multiple name="userName[]" id="userName">
                        @foreach($users as $user)
                            ...
                            @foreach($roleUsers as $roleUser)
                                @if($user->user_id === $roleUser->user_id)
                                    <?php $selected = true;?>
                                @endif
                            @endforeach
                            <option value="{{$user->user_id}}" @if($selected) selected @endif>{{$user->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                                                        ...
                                                        <li>
                                                            <div class="i-checks">
                                                                @foreach($rolePermissions as $rolePermission)
                                                                    @if($permission['permission_id'] == $rolePermission->permission_id)
                                                                        <?php $checked = true;?>
                                                                    @endif
                                                                @endforeach
                                                                <input type="checkbox" class="i-checks" @if($checked) checked @endif name="permissions[]" value="{{$permission['permission_id']}}">{{$permission['desc']}}
                                                            <div>
                                                        </li>
                                                    @endforeach
                                                @endif
                                                                    ...
                                                                    @foreach($permissions[$key]['edit']['detail'] as $filed)
                                                                        ...
                                                                        <li>
                                                                            <div class="checkbox m-l m-r-xs">
                                                                                @foreach($rolePermissions as $rolePermission)
                                                                                    @if($filed['permission_id'] == $rolePermission->permission_id)
                                                                                        <?php $checked = true;?>
                                                                                    @endif
                                                                                @endforeach
                                                                                <input type="checkbox" name="permissions[]" @if($checked) checked @endif class="i-checks" value="{{$filed['permission_id']}}">{{$filed['desc']}}
                                                                            </div>
                                                                        </li>
                                                                    @endforeach
                                                                </ul>
